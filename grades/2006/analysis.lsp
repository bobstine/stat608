...
(def grades (read-grades "scan.dat" :name 0 :ssn 27 :ques 43 :nques 44))
...
(def keys
     (list
       '(a  ; dummy first key since this exam did not have one
         d b a e d c b e e c         ;  1-
         b c c b c a e d b d         ; 11-
         b d a c c e b c e b         ; 21-
         a c c c e c d b c c         ; 31-
         e a e c )                   ; 41-
      ))
                             
(def keys  ; these are easier to improve the scores
     (list
       '(a  
         d b a e d c b e e c         ;  1-
         b c c b c a e d * d         ; 11-
         b d a c c (a e) b c e b     ; 21-
         a c c c (c e) c d b c *     ; 31-
         e a e c )                   ; 41-
      ))
...
(with-open-file (f "forjmp.txt" :direction :output)
                (format f "LastName\tFirstName\tExam\tSSN\tScore\tnCorrect\tTrueCount~%")
                (dolist (r (transpose (list (select lastNames  index)
					    ...
					    (select trueCount  index)) ))
		  (apply #'format f "~15a\t~15a\t~a\t~10a\t~5,2f\t~d\t~d~%" r)
		  ))
